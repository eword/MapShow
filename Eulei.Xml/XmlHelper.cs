...
namespace Eulei.Serialization
{
    ...
    public class XmlHelper
    {
        ...
        private XmlDocument GetXmlDocument()
        {
            ...
            {
                doc = GetXmlDocumentFromFile(AppDomain.CurrentDomain.BaseDirectory + xmlFilePath);
            }
            return doc;
        }
        ...
        public void SaveXmlDocument()
        {
            try
            {
                ...
                {
                    Savexml(AppDomain.CurrentDomain.BaseDirectory + xmlFilePath);
                }
            }
            catch (XmlException xmle)
            {
        ...
        #region DataSet保存XML文件----------------------------------------------------
        /// <summary>
        /// 功能: 
        /// 保存XML文件
        /// </summary>
        public void DataSetSaveXmlDocument(DataSet _dataSet)
        {
            try
            {
                //保存设置的结果
                if (this.xmlFilePathType == enumXmlPathType.AbsolutePath)
                {
                    _dataSet.WriteXmlSchema(xmlFilePath);
                }
                else if (this.xmlFilePathType == enumXmlPathType.VirtualPath)
                {
                    _dataSet.WriteXmlSchema(AppDomain.CurrentDomain.BaseDirectory + xmlFilePath);
                }
            }
            catch (XmlException xmle)
            {
        ...
        #region DataSet读取XML文件----------------------------------------------------
        /// <summary>
        /// 功能: 
        /// 保存XML文件
        /// </summary>
        public DataSet GetXmlDocumentToDataSet()
        {
            ...
            try
            {
                ...
                if (this.xmlFilePathType == enumXmlPathType.AbsolutePath)
                {
                    _dataSet.ReadXml(xmlFilePath);
                }
                else if (this.xmlFilePathType == enumXmlPathType.VirtualPath)
                {
                    _dataSet.ReadXml(AppDomain.CurrentDomain.BaseDirectory + xmlFilePath);
                }
